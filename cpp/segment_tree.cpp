...
{
	//required seg_tree node count is 2*2^(floor(log2(N)) + 1);
	int length=(int)(2*pow(2.0,floor((log(n)/log(2.0)+1))));
...
void build_segment_tree(int code,int * A,int node,int b,int e){
	if(b==e){
		if(code==RANGE_SUM)
			segment_tree[node]=A[b];//store value of cell
		else
			segment_tree[node]=b;//if range_max/min store index
	}
	else
	{
		//recurrsivly compute the values of left and right subtrees
		int leftIdx = 2 * node;//dawa child
		int rightIdx = 2 * node + 1;//ujwa child
...
int query(int code, int A[], int node, int b, int e, int i, int j) {
	if (i > e || j < b) return -1; // if the current interval does not intersect query interval
		...
		int p1 = query(code, A, 2 * node , b , (b + e) / 2, i, j);//left
		int p2 = query(code, A, 2 * node + 1, (b + e) / 2 + 1, e , i, j);//right
...
int main() {
	...
	for(int i=0;i<segment_tree.size();i++)
		cout<<segment_tree[i]<<" "<<A[segment_tree[i]]<<endl;
	return 0;
}
